...
import {
  ...
  Menu, Bell, LogOut, Star, Filter, ChevronDown,
...
const categoryColors = {
  Verduras: { bg: "bg-green-primary/10", text: "text-green-primary", dot: "bg-green-primary" },
  Granos: { bg: "bg-orange-brand/10", text: "text-orange-brand", dot: "bg-orange-brand" },
  Frutas: { bg: "bg-teal-brand/10", text: "text-teal-brand", dot: "bg-teal-brand" },
};
...
function ProductCard({ product, onAddToCart, cartCount }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Big emoji as product visual */}
        <div className="text-6xl select-none" style={{ filter: "drop-shadow(0 4px 12px rgba(0,0,0,0.1))" }}>
          {product.category === "Verduras" ? "🥬" : product.category === "Granos" ? "🌽" : "🍋"}
        </div>
        ...
        {cartCount > 0 && (
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="absolute top-3 right-3 w-6 h-6 rounded-full bg-orange-brand flex items-center justify-center"
          >
      ...
      <div className="p-4">
        {/* Category */}
        <div className={`badge ${c.bg} ${c.text} mb-2`}>
          <span className={`w-1 h-1 rounded-full ${c.dot}`}/>
          {product.category}
        </div>

        {/* Name & origin */}
        <h3 className="font-body font-semibold text-earth-dark text-sm leading-tight mb-0.5">
          {product.name}
        </h3>
        ...
        <div className="flex items-end justify-between mb-3">
          <div>
            <span className="font-display font-bold text-earth-dark text-lg">
          ...
          <div className="text-right">
            ...
            <div className="font-body text-earth-tan text-xs">disponibles</div>
          </div>
        </div>

        {/* Producer + rating */}
        <div className="flex items-center gap-2 mb-4 pb-3 border-b border-cream-dark">
          <div className="w-6 h-6 rounded-full bg-green-primary flex items-center justify-center text-cream text-xs font-bold flex-shrink-0">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 bg-green-primary text-cream font-body font-semibold text-sm py-2.5 rounded-xl hover:bg-green-dark transition-all duration-200 hover:-translate-y-0.5 shadow-card"
        >
          <Plus size={15}/>
          Agregar al pedido
        </button>
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-cream-dark">
          <div className="flex items-center gap-2">
            <ShoppingCart size={20} className="text-green-primary"/>
            <h3 className="font-display font-bold text-earth-dark text-lg">Mi Pedido</h3>
            <span className="badge bg-orange-brand/10 text-orange-brand">{cartItems.length} productos</span>
          </div>
          <button onClick={onClose} className="p-2 rounded-lg text-earth-tan hover:bg-cream-dark transition-colors">
            <X size={18}/>
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin">
          {cartItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-40 text-center">
              <ShoppingCart size={32} className="text-earth-tan mb-3"/>
              <p className="font-body text-earth-tan text-sm">Tu pedido está vacío</p>
              <p className="font-body text-earth-tan text-xs mt-1">Agrega productos desde el catálogo</p>
            </div>
          ) : (
            cartItems.map((item) => (
              <div key={item.id} className="flex items-center gap-3 bg-cream/50 rounded-2xl p-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-2xl" style={{ backgroundColor: item.color + "22" }}>
                  {item.category === "Verduras" ? "🥬" : item.category === "Granos" ? "🌽" : "🍋"}
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => onUpdateQty(item.id, -1)} className="w-7 h-7 rounded-full bg-cream-dark flex items-center justify-center text-earth-brown hover:bg-cream-darker transition-colors">
                    <Minus size={12}/>
                  </button>
                  <span className="font-body font-bold text-earth-dark text-sm w-6 text-center">{item.qty}</span>
                  <button onClick={() => onUpdateQty(item.id, 1)} className="w-7 h-7 rounded-full bg-green-primary/10 flex items-center justify-center text-green-primary hover:bg-green-primary/20 transition-colors">
                    <Plus size={12}/>
                  </button>
                </div>
                <div className="font-body font-semibold text-green-primary text-sm text-right w-16">
        ...
        {cartItems.length > 0 && (
          <div className="p-4 border-t border-cream-dark space-y-3">
            <div className="flex items-center justify-between">
              <span className="font-body text-earth-brown text-sm">Subtotal</span>
            ...
            <div className="flex items-center gap-2 bg-green-primary/8 rounded-xl p-3">
              <Truck size={16} className="text-green-primary flex-shrink-0"/>
              <span className="font-body text-green-primary text-xs font-medium">Entrega garantizada en 24h con cadena de frío</span>
            </div>
            <button className="w-full bg-orange-brand text-white font-body font-semibold py-3.5 rounded-xl hover:bg-orange-dark transition-colors shadow-orange text-sm">
              Confirmar pedido →
            </button>
          </div>
        )}
    ...
    <div className="min-h-screen bg-cream flex">
      ...
      >
        ...
        <div className="p-5 border-b border-cream-dark">
          <Link to="/" className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="font-body text-earth-tan text-xs">Panel Comprador</div>
            </div>
          </Link>
        </div>

        {/* User info */}
        <div className="p-4 mx-3 mt-3 bg-teal-brand/8 rounded-2xl border border-teal-brand/15">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-teal-brand flex items-center justify-center text-cream font-display font-bold text-sm">
              MV
            </div>
            <div>
              <div className="font-body font-semibold text-earth-dark text-sm">La Milpa Rest.</div>
        ...
        <nav className="flex-1 p-3 space-y-1 mt-2">
          {navItems.map(({ key, label, icon: Icon }) => (
            ...
            >
              ...
              <span>{label}</span>
              {activeNav === key && <ChevronRight size={14} className="ml-auto"/>}
            </button>
          ))}
        </nav>
        ...
        <div className="p-3 border-t border-cream-dark">
          <Link to="/" className="sidebar-item w-full text-left text-earth-brown hover:text-red-500 hover:bg-red-50">
            <LogOut size={17}/>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-20 bg-white/95 backdrop-blur-md border-b border-cream-dark px-4 md:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button className="lg:hidden p-2 rounded-lg text-earth-brown hover:bg-cream-dark" onClick={() => setSidebarOpen(true)}>
              <Menu size={20}/>
            </button>
            <div>
              ...
              <p className="font-body text-earth-tan text-xs">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="relative flex items-center gap-2 bg-orange-brand text-white font-body font-semibold text-sm px-4 py-2 rounded-full hover:bg-orange-dark transition-colors shadow-orange"
            >
              <ShoppingCart size={16}/>
              <span className="hidden sm:inline">Mi pedido</span>
              {cartCount > 0 && (
                <span className="w-5 h-5 rounded-full bg-white text-orange-brand text-xs font-bold flex items-center justify-center">
                  {cartCount}
                </span>
              )}
            </button>
        ...
        <div className="flex-1 p-4 md:p-8">
            ...
            <>
              ...
              <div className="mb-6 space-y-3">
                ...
                <div className="relative">
                  <Search size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-earth-tan"/>
                  <input
                    type="text"
                    placeholder="Buscar productos, productores, regiones..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="w-full bg-white border border-cream-darker rounded-2xl pl-11 pr-4 py-3 font-body text-sm text-earth-dark focus:outline-none focus:ring-2 focus:ring-teal-brand focus:border-transparent placeholder:text-earth-tan shadow-card"
                  />
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {/* Category filter */}
                  <div className="flex items-center gap-1.5 bg-white rounded-xl px-1 py-1 shadow-sm border border-cream-dark">
                    {categories.map((cat) => (
                      <button
                        key={cat}
                        onClick={() => setActiveCategory(cat)}
                        className={`font-body text-xs font-medium px-3 py-1.5 rounded-lg transition-all duration-200 ${
                          activeCategory === cat
                            ? "bg-green-primary text-cream shadow-sm"
                            : "text-earth-brown hover:bg-cream-dark"
                        }`}
                      >
                        {cat}
                      </button>
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="appearance-none bg-white border border-cream-dark rounded-xl px-4 py-2 font-body text-xs text-earth-brown font-medium focus:outline-none focus:ring-2 focus:ring-teal-brand shadow-sm pr-7 cursor-pointer"
                    >
                      {states.map((s) => (
              ...
              <div className="flex items-center gap-2 mb-4">
                <Package size={16} className="text-earth-tan"/>
                ...
                {(activeCategory !== "Todas" || activeState !== "Todo México" || searchQuery) && (
                  <button
                    ...
                    className="ml-2 text-xs text-teal-brand font-medium hover:text-teal-light transition-colors flex items-center gap-1"
                  >
                    <X size={12}/>
                    Limpiar filtros
                  </button>
              ...
              {/* Product grid */}
              <motion.div layout className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-4 md:gap-5">
                <AnimatePresence>
              ...
              {filteredProducts.length === 0 && (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="font-body text-earth-tan text-sm max-w-xs">
                    No encontramos productos con esos filtros. Intenta con otra categoría o región.
                  </p>
                </div>
              )}
            </>
          ...
          {activeNav !== "explorar" && (
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <button onClick={() => setActiveNav("explorar")} className="mt-4 btn-outline text-sm">
                Explorar productos
              </button>
            </div>
          )}
